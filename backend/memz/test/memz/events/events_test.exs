defmodule Memz.EventsTest do
  ...
  describe "events" do
    ...
    test "create_event/1 with a name that is too short returns error changeset" do
      ...
      assert {"should be at least %{count} character(s)", [count: 4, validation: :length, min: 4]} =
    ...
    test "create_event/1 with an end_date which is not a date string" do
      ...
      assert {"is invalid", [type: :naive_datetime, validation: :cast]} =
               changeset.errors[:end_date]
    end
