defmodule MemzWeb.EventChannelTest do
  ...
  describe "joining an event channel" do
    ...
    test "it should deny access if you don't give the token for the owner of the event", %{
      token: token,
      event_id: event_id
    } do
      {:error, %{reason: reason}} =
        socket()
        |> subscribe_and_join(EventChannel, "event:" <> event_id, %{
             "guardian_token" => "not_event_owner_token"
           })

      assert reason == "unauthorized"
    end

    test "it should reply with the list of images in the event on join", %{
      token: token,
      event_id: event_id,
      images: images
    } do
